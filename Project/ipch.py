...
def main():
    ips = list(set([BoxNetworkIp(ip) for ip in slurp_unknown_ips()]))
    ...
    # for k, v in bucket['g3']['compute'].items():
    #     print(k, len(v))
    if args['output'] == 'dict':
        pprint(bucket)
    else:
        flat_bucket = flatten(bucket)
        x = PrettyTable(["Type Based on IP", "IP Address"])
        x.align["Type Based on IP"] = "l"
        x.align["IP Address"] = "l"
        x.padding_width = 1 # One space between column edges and contents (default)
        for k, v in flat_bucket.items():
            if len(v):
                for ip in v:
                    x.add_row([k, ip])
        print(x)
    print('super unknown', len(bucket['unknown']))
    print('g2 compute unknown', len(bucket['g2']['compute']['unknown']))
    print('g3 unknown', len(bucket['g3']['unknown']))
    print('g3 compute unknown', len(bucket['g3']['compute']['unknown']))
    print('edc unknown', len(bucket['edc']['unknown']))
    print('edc oob unknown', len(bucket['edc']['oob']['unknown']))
    print('edc compute unknown', len(bucket['edc']['compute']['unknown']))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Description of your program')
    parser.add_argument('-o', '--output', default='dict', help='dict|col', required=False)
    parser.add_argument('-s', '--slurpfile', default='all_unknown.txt', help='filename of ips to slurp', required=False)
